...
function startAdapter(options) {
    ...
    adapter.on('unload', function (callback) {
        try {
            if (checkAliveTimeoutTimer) {
                clearTimeout(checkAliveTimeoutTimer);
            }
            if (checkAliveTimer) {
                clearTimeout(checkAliveTimer);
            }
            if (sendDelayTimer) {
...
function main() {
    ...
    adapter.getStates('*', function (err, obj) {
        if (err) {
            error(`Error reading states: ${err}`);
        } else {
            if (obj) {
                for (var key in obj) {
            ...
            adapter.setObjectNotExists(`${adapter.namespace}.${CONTROLLER_RESEACH_DEVICES}`, {
                type: 'state',
                common: { name: CONTROLLER_RESEACH_DEVICES, type: 'boolean', role: 'button' },
                native: {},
            });
            adapter.setObjectNotExists(`${adapter.namespace}.${CONTROLLER_ADMIN_FUNCTION}`, {
                type: 'state',
                common: { name: CONTROLLER_ADMIN_FUNCTION, type: 'string', role: 'switch' },
                native: {},
            });
...
function readFirmwareVersionFor(firmwareId, firmwareName) {
    ...
    http.request(options, function (response) {
        ...
        response.on('end', function () {
            ...
            if (typeof onlineVersions[firmwareId] == 'undefined') {
                onlineVersions[firmwareId] = {};
            }
            ...
            for (var deviceId in firmwareTypes) {
                ...
                if (myFirmwareId == firmwareId) {
                    var myId = getIoBrokerId(
                        deviceId,
                        CLASS_ID_CONTROLLER,
                        1,
                        CONTROLLER_CFG_NEWEST_FIRMWARE,
                        CHANNEL_CONFIG,
                    );
    ...
    if (stateSize != 0) {
        ...
        if (checkAliveControllerDeviceId) {
            ...
            for (var id in ioBrokerStates) {
                if (!id.includes(`.${CONTROLLER_FKT_STATE_ONLINE}`)) {
                    continue;
                }
        ...
        if (newCheckControllerDeviceId) {
            ...
            checkAliveTimeoutTimer = setTimeout(function () {
                checkAliveTimeoutOccured(newCheckControllerIoBrokerId);
            }, 3000); // offline nach 3 Sekunden
...
function sendNextQueueDatagram() {
    ...
    if (queueControl.length > 3) {
        ...
        sendDelayTimer = setInterval(sendNextQueueDatagram, 10);
    }
...
function handleIncomingMessage(message, remote) {
    ...
    try {
        ...
        debug(
            `sender: ${sender}, receiver: ${receiver}, functionId: ${functionId}, dataLength: ${
                dataLength
            }, classIdSender: ${classIdSender}`,
        );

        checkAliveOk(senderDeviceId);

        if (classIdSender != CLASS_ID_CONTROLLER) {
            checkUnknownDeviceId(senderDeviceId);
        }
        ...
        if (classIdSender == CLASS_ID_CONTROLLER) {
            if (functionId == 128) {
                hwControllerReceivedModuleId(sender, receiver, message, dataLength);
            } else if (functionId == 129) {
                hwControllerReceivedRemoteObjects(sender, receiver, message, dataLength);
            } else if (functionId == 202) {
                hwControllerReceivedEvStarted(sender);
            } else if (functionId == 131) {
                hwControllerReceivedConfiguration(sender, receiver, message, dataLength);
            } else if (functionId == 199) {
                hwControllerReceivedPong(sender);
            }
        } else if (classIdSender == CLASS_ID_FEUCHTESENSOR) {
            if (functionId == 128) {
                hwFeuchteSensorReceivedConfiguration(sender, receiver, message, dataLength);
            } else if (functionId == 129 || functionId == 203) {
                hwFeuchteSensorReceivedStatus(sender, receiver, message, dataLength);
            } else if (functionId >= 200 && functionId < 300) {
                hwFeuchteSensorReceivedEvents(sender, receiver, functionId, message, dataLength);
            }
        } else if (classIdSender == CLASS_ID_ANALOGEINGANG) {
            if (functionId == 128) {
                hwAnalogInputReceivedConfiguration(sender, receiver, message, dataLength);
            } else if (functionId == 129 || functionId == 203) {
                hwAnalogInputReceivedStatus(sender, receiver, message, dataLength);
            }
            ...
            if (functionId == 128) {
                hwPowerMeterReceivedConfiguration(sender, receiver, message, dataLength);
            }
            if (functionId == 129 || functionId == 203) {
                hwPowerMeterReceivedStatus(sender, receiver, message, dataLength);
            }
            //else if (functionId>=200 && functionId<300) hwAnalogInputReceivedEvents(sender, receiver, functionId, message, dataLength);
        } else if (classIdSender == CLASS_ID_HELLIGKEITSSENSOR) {
            if (functionId == 128) {
                hwHelligkeitssensorReceivedConfiguration(sender, receiver, message, dataLength);
            } else if (functionId == 129 || functionId == 203) {
                hwHelligkeitssensorReceivedStatus(sender, receiver, message, dataLength);
            } else if (functionId >= 200 && functionId < 300) {
                hwHelligkeitssensorReceivedEvents(sender, receiver, functionId, message, dataLength);
            }
        } else if (classIdSender == CLASS_ID_LED) {
            if (functionId == 128) {
                hwLedReceivedConfiguration(sender, receiver, message, dataLength);
            } else if (functionId == 129) {
                hwLedReceivedStatus(sender, receiver, message, dataLength);
            } else if (functionId >= 200 && functionId < 300) {
                hwLedReceivedEvents(sender, receiver, functionId, message, dataLength);
            }
        } else if (classIdSender == CLASS_ID_LOGICAL_BUTTON) {
            if (functionId == 129) {
                hwLogicalButtonReceivedStatus(sender, receiver, message, dataLength);
            } else if (functionId >= 200 && functionId < 300) {
                hwLogicalButtonReceivedEvents(sender, receiver, functionId, message, dataLength);
            }
        } else if (classIdSender == CLASS_ID_SCHALTER) {
            if (functionId == 128) {
                hwSchalterReceivedConfiguration(sender, receiver, message, dataLength);
            } else if (functionId == 129) {
                hwSchalterReceivedStatus(sender, receiver, message, dataLength);
            } else if (functionId >= 200 && functionId < 300) {
                hwSchalterReceivedEvents(sender, receiver, functionId, message, dataLength);
            }
        } else if (classIdSender == CLASS_ID_TASTER) {
            if (functionId == 128) {
                hwTasterReceivedConfiguration(sender, receiver, message, dataLength);
            } else if (functionId == 129) {
                hwTasterReceivedStatus(sender, receiver, message, dataLength);
            } else if (functionId >= 200 && functionId < 300) {
                hwTasterReceivedEvents(sender, receiver, functionId, message, dataLength);
            }
        } else if (classIdSender == CLASS_ID_ROLLLADEN) {
            if (functionId == 128) {
                hwRolloReceivedConfiguration(sender, receiver, message, dataLength);
            } else if (functionId == 129) {
                hwRolloReceivedStatus(sender, receiver, message, dataLength);
            } else if (functionId >= 200 && functionId < 300) {
                hwRolloReceivedEvents(sender, receiver, functionId, message, dataLength);
            }
        } else if (classIdSender == CLASS_ID_DIMMER) {
            if (functionId == 128) {
                hwDimmerReceivedConfiguration(sender, receiver, message, dataLength);
            } else if (functionId == 129) {
                hwDimmerReceivedStatus(sender, receiver, message, dataLength);
            } else if (functionId >= 200 && functionId < 300) {
                hwDimmerReceivedEvents(sender, receiver, functionId, message, dataLength);
            }
        } else if (classIdSender == CLASS_ID_RGB_DIMMER) {
            if (functionId == 128) {
                hwRgbDimmerReceivedConfiguration(sender, receiver, message, dataLength);
            } else if (functionId == 129) {
                hwRgbDimmerReceivedStatus(sender, receiver, message, dataLength);
            } else if (functionId >= 200 && functionId < 300) {
                hwRgbDimmerReceivedEvents(sender, receiver, functionId, message, dataLength);
            }
            ...
            if (functionId == 128) {
                hwTemperatursensorReceivedConfiguration(sender, receiver, message, dataLength);
            } else if (functionId == 129 || functionId == 203) {
                hwTemperatursensorReceivedStatus(sender, receiver, message, dataLength);
            } else if (functionId >= 200 && functionId < 300) {
                hwTemperatursensorReceivedEvents(sender, receiver, functionId, message, dataLength);
            }
        } else if (classIdSender == CLASS_ID_ETHERNET) {
            if (functionId == 129) {
                hwEthernetReceivedIp(sender, receiver, message, dataLength);
            } else if (functionId == 128) {
                hwEthernetReceivedConfiguration(sender, receiver, message, dataLength);
            }
        }
    } catch (e) {
...
function aFunctionCall(ioBrokerId, newValue) {
    ...
    if (classId == CLASS_ID_SCHALTER) {
        if (state == SCHALTER_FKT_ON_OFF) {
            if (newValue == true) {
                hwSchalterOn(0, 0, objectId);
            } else if (newValue == false) {
                hwSchalterOff(0, objectId);
            }
        ...
        } else if (state == SCHALTER_FKT_OFF_DELAY) {
            hwSchalterOff(newValue, objectId);
        }
    } else if (classId == CLASS_ID_TASTER) {
        if (state == TASTER_FKT_ENABLE_DISABLE_EVENTS) {
            hwTasterEnableEvents(newValue ? 'TRUE' : 'FALSE', 0, objectId);
        } else if (state == TASTER_FKT_DISABLE_EVENTS_TIMEOUT) {
            hwTasterEnableEvents('FALSE', newValue, objectId);
        } else if (
            state == TASTER_CFG_HOLD_TIMEOUT ||
        ...
        if (state == ROLLO_FKT_POSITION) {
            hwRolloMoveToPosition(newValue, objectId);
        } else if (state == ROLLO_FKT_START) {
            hwRolloStart(newValue, objectId);
        } else if (state == ROLLO_FKT_TOGGLE) {
            hwRolloToggle(objectId);
        } else if (state == ROLLO_FKT_STOP) {
            hwRolloStop(objectId);
        } else if (state == ROLLO_CFG_SET_POSITION) {
            hwRolloSetPosition(newValue, objectId);
            hwRolloGetStatus(objectId);
        } else if (state == ROLLO_FKT_UPDATE_POSITION) {
            hwRolloGetStatus(objectId);
        } else if (state == ROLLO_CFG_CLOSE_TIME || state == ROLLO_CFG_OPEN_TIME || state == ROLLO_CFG_INVERT) {
            hwRolloSetConfiguration(state, newValue, objectId);
        }
    } else if (classId == CLASS_ID_DIMMER) {
        if (state == DIMMER_FKT_BRIGHTNESS) {
            hwDimmerSetBrightness(newValue, 0, objectId);
        ...
        } else if (state == DIMMER_FKT_START) {
            hwDimmerStart(newValue, objectId);
        } else if (state == DIMMER_FKT_STOP) {
            hwDimmerStop(objectId);
        } else if (state == DIMMER_FKT_TOGGLE) {
            hwDimmerToggle(objectId);
        } else if (
            state == DIMMER_CFG_DIMMING_RANGE_END ||
        ...
        } else if (state == RGB_DIMMER_CFG_FADING_TIME) {
            hwRgbDimmerSetConfiguration(state, newValue, objectId);
        }
    } else if (classId == CLASS_ID_CONTROLLER) {
        if (state == CONTROLLER_FKT_RESET) {
            hwControllerReset(objectId);
        } else if (state == ETHERNET_CFG_FIXED_IP_DHCP) {
            hwEthernetSetConfiguration(newValue, getObjectId(deviceId, CLASS_ID_ETHERNET, 1));
        } else if (state == CONTROLLER_CFG_UPDATE_FIRMWARE) {
            if (newValue == 'UPDATE') {
                hwControllerUpdateFirmware(objectId);
            } else {
                error("Invalid value. To update firmware please enter 'UPDATE'.");
            }
        }
    } else if (classId == CLASS_ID_LED) {
        if (state == LED_FKT_ON_OFF) {
            if (newValue == true) {
                hwLedOn(100, 0, 0, objectId);
            } else if (newValue == false) {
                hwLedOff(0, objectId);
            }
        } else if (state == LED_CFG_INVERTED) {
            hwLedSetConfiguration(state, newValue, objectId);
        } else if (state == LED_CFG_TIMEBASE) {
            hwLedSetConfiguration(state, newValue, objectId);
        ...
        } else if (state == LED_FKT_OFF_DELAY) {
            hwLedOff(newValue, objectId);
        } else if (state == LED_FKT_MIN_BRIGHTNESS) {
            hwLedMinBrightness(newValue, objectId);
            ...
            if (brightness == 0) {
                ...
                hwLedBlink(brightness, offTime, onTime, quantity, objectId);
            }
        }
    } else if (classId == CLASS_ID_LOGICAL_BUTTON) {
        if (state == LOGICAL_BUTTON_FKT_ON_OFF) {
            if (newValue == true) {
                hwLogicalButtonOn(100, 0, objectId);
            } else if (newValue == false) {
                hwLogicalButtonOff(objectId);
            }
            ...
            if (newValue == 0) {
                ...
                hwLogicalButtonOn(newValue, 0, objectId);
            }
            hwLedGetStatus(objectId);
        } else if (state == LOGICAL_BUTTON_FKT_MIN_BRIGHTNESS) {
            hwLogicalButtonSetMinBrightness(newValue, 0, objectId);
            ...
            if (brightness == 0) {
                hwLogicalButtonOff(0, objectId);
            } else {
                hwLogicalButtonBlink(brightness, offTime, onTime, quantity, objectId);
            }
        }
    } else if (classId == CLASS_ID_TEMPERATURSENSOR) {
        if (
            state == TEMP_CFG_CALIBRATION ||
        ...
        if (
            state == HUMIDITY_CFG_CALIBRATION ||
...
function hwTemperatursensorReceivedStatus(sender, receiver, message, dataLength) {
    ...
    var state = '';
    if (byteLastEvent == 200) {
        state = 'COLD';
    } else if (byteLastEvent == 201) {
        state = 'NORMAL';
    }
    if (byteLastEvent == 202) {
        state = 'HOT';
    }
...
function hwTemperatursensorReceivedConfiguration(sender, receiver, message, dataLength) {
    ...
    var myId = getIoBrokerId(
        deviceId,
        CLASS_ID_TEMPERATURSENSOR,
        instanceId,
        TEMP_CFG_REPORT_TIME_MULTIPLIER,
        CHANNEL_CONFIG,
    );
...
function hwTemperatursensorSetConfiguration(configKey, newValue, receiverObjectId, recovery = '0') {
    ...
    if (typeof configuration != 'undefined') {
        ...
        setTimeout(function () {
            hwTemperatursensorSetConfiguration(configKey, newValue, receiverObjectId, '1');
        }, 1000);
        return;
    }

    if (configKey == TEMP_CFG_CALIBRATION) {
        calibration = parseInt(newValue * 10);
    } else if (configKey == TEMP_CFG_HYSTERESIS) {
        hysteresis = parseInt(newValue * 10);
    } else if (configKey == TEMP_CFG_MAX_REPORT_TIME) {
        maxReportTime = parseInt(newValue);
    } else if (configKey == TEMP_CFG_MIN_REPORT_TIME) {
        minReportTime = parseInt(newValue);
    } else if (configKey == TEMP_CFG_REPORT_TIME_MULTIPLIER) {
        reportTimeBase = parseInt(newValue);
    ...
    debug(
        `temperaturSensorSetConfiguration: ${dump(configurations[receiverObjectId])} -> ${objectIdToString(
            receiverObjectId,
        )}`,
    );
}
...
function hwHelligkeitssensorReceivedConfiguration(sender, receiver, message, dataLength) {
    ...
    var myId = getIoBrokerId(
        deviceId,
        CLASS_ID_HELLIGKEITSSENSOR,
        instanceId,
        BRIGHT_CFG_LOWER_THRESHOLD,
        CHANNEL_CONFIG,
    );
    setStateIoBroker(myId, `${lowerThreshold}.${lowerThresholdFraction}`);
    ...
    var myId = getIoBrokerId(
        ...
        CLASS_ID_HELLIGKEITSSENSOR,
        instanceId,
        BRIGHT_CFG_REPORT_TIME_MULTIPLIER,
        CHANNEL_CONFIG,
    );
    setStateIoBroker(myId, reportTimeBase);
...
function hwHelligkeitssensorSetConfiguration(configKey, newValue, receiverObjectId, recovery = '0') {
    ...
    if (typeof configuration != 'undefined') {
        ...
        setTimeout(function () {
            hwHelligkeitssensorSetConfiguration(configKey, newValue, receiverObjectId, '1');
        }, 1000);
        return;
    }

    if (configKey == BRIGHT_CFG_CALIBRATION) {
        calibration = parseInt(newValue * 10);
    } else if (configKey == BRIGHT_CFG_HYSTERESIS) {
        hysteresis = parseInt(newValue * 10);
    } else if (configKey == BRIGHT_CFG_MAX_REPORT_TIME) {
        maxReportTime = parseInt(newValue);
    } else if (configKey == BRIGHT_CFG_MIN_REPORT_TIME) {
        minReportTime = parseInt(newValue);
    } else if (configKey == BRIGHT_CFG_REPORT_TIME_MULTIPLIER) {
        reportTimeBase = parseInt(newValue);
    ...
    info(
        `brightness sensor setConfiguration: ${dump(configurations[receiverObjectId])} -> ${objectIdToString(
            receiverObjectId,
        )}`,
    );
}
...
function hwFeuchteSensorReceivedConfiguration(sender, receiver, message, dataLength) {
    ...
    var myId = getIoBrokerId(
        deviceId,
        CLASS_ID_FEUCHTESENSOR,
        instanceId,
        HUMIDITY_CFG_LOWER_THRESHOLD,
        CHANNEL_CONFIG,
    );
    setStateIoBroker(myId, `${lowerThreshold}.${lowerThresholdFraction}`);
    ...
    var myId = getIoBrokerId(
        ...
        CLASS_ID_FEUCHTESENSOR,
        instanceId,
        HUMIDITY_CFG_REPORT_TIME_MULTIPLIER,
        CHANNEL_CONFIG,
    );
    setStateIoBroker(myId, reportTimeBase);
...
function hwFeuchteSensorSetConfiguration(configKey, newValue, receiverObjectId, recovery = '0') {
    ...
    if (typeof configuration != 'undefined') {
        ...
        setTimeout(function () {
            hwFeuchteSensorSetConfiguration(configKey, newValue, receiverObjectId, '1');
        }, 1000);
        return;
    }

    if (configKey == HUMIDITY_CFG_CALIBRATION) {
        calibration = parseInt(newValue * 10);
    } else if (configKey == HUMIDITY_CFG_HYSTERESIS) {
        hysteresis = parseInt(newValue * 10);
    } else if (configKey == HUMIDITY_CFG_MAX_REPORT_TIME) {
        maxReportTime = parseInt(newValue);
    } else if (configKey == HUMIDITY_CFG_MIN_REPORT_TIME) {
        minReportTime = parseInt(newValue);
    } else if (configKey == HUMIDITY_CFG_REPORT_TIME_MULTIPLIER) {
        reportTimeBase = parseInt(newValue);
    ...
    info(
        `humidity sensor setConfiguration: ${dump(configurations[receiverObjectId])} -> ${objectIdToString(
            receiverObjectId,
        )}`,
    );
}
...
function hwAnalogInputReceivedConfiguration(sender, receiver, message, dataLength) {
    ...
    var myId = getIoBrokerId(
        deviceId,
        CLASS_ID_ANALOGEINGANG,
        instanceId,
        ANALOG_CFG_REPORT_TIME_MULTIPLIER,
        CHANNEL_CONFIG,
    );
    setStateIoBroker(myId, reportTimeBase);
...
function hwAnalogInputSetConfiguration(configKey, newValue, receiverObjectId, recovery = '0') {
    ...
    if (typeof configuration != 'undefined') {
        ...
        setTimeout(function () {
            hwAnalogInputSetConfiguration(configKey, newValue, receiverObjectId, '1');
        }, 1000);
        return;
    }

    if (configKey == ANALOG_CFG_CALIBRATION) {
        calibration = parseInt(newValue);
    } else if (configKey == ANALOG_CFG_HYSTERESIS) {
        hysteresis = parseInt(newValue);
    } else if (configKey == ANALOG_CFG_MAX_REPORT_TIME) {
        maxReportTime = parseInt(newValue);
    } else if (configKey == ANALOG_CFG_MIN_REPORT_TIME) {
        minReportTime = parseInt(newValue);
    } else if (configKey == ANALOG_CFG_REPORT_TIME_MULTIPLIER) {
        reportTimeBase = parseInt(newValue);
    }
    ...
    info(
        `anlogInput setConfiguration: ${dump(configurations[receiverObjectId])} -> ${objectIdToString(
            receiverObjectId,
        )}`,
    );
}
...
function hwPowerMeterSetConfiguration(configKey, newValue, receiverObjectId, recovery = '0') {
    ...
    if (typeof configuration != 'undefined') {
        ...
        setTimeout(function () {
            hwPowerMeterSetConfiguration(configKey, newValue, receiverObjectId, '1');
        }, 1000);
        return;
    }
    ...
    info(
        `hwPowerMeterSetConfiguration setConfiguration: ${dump(configurations[receiverObjectId])} -> ${objectIdToString(
            receiverObjectId,
        )}`,
    );
}
...
function hwTasterEnableEvents(enable, disableDuration, receiverObjectId) {
    info(
        `input enableEvents enable = ${enable}, disableDuration = ${disableDuration} -> ${objectIdToString(
            receiverObjectId,
        )}`,
    );
...
function hwTasterReceivedEvents(sender, receiver, functionId, message, dataLength) {
    ...
    var event = '';
    if (functionId == 200) {
        event = TASTER_FKT_PRESSED;
    } else if (functionId == 201) {
        event = TASTER_FKT_CLICKED;
    } else if (functionId == 202) {
        event = TASTER_FKT_DOUBLE_CLICKED;
    } else if (functionId == 203) {
        event = TASTER_FKT_HOLD_START;
    } else if (functionId == 204) {
        event = TASTER_FKT_HOLD_END;
    } else if (functionId == 205) {
        event = TASTER_FKT_RELEASED;
    }
...
function hwTasterReceivedConfiguration(sender, receiver, message, dataLength) {
    ...
    configurations[sender] = {
        ...
        doubleClickTimeout: waitForDoubleClickTimeout,
        events: byteEvents,
        options: byteOptions,
    };
...
function hwTasterSetConfiguration(configKey, newValue, receiverObjectId, recovery = '0') {
    ...
    if (typeof configuration != 'undefined') {
        ...
        setTimeout(function () {
            hwTasterSetConfiguration(configKey, newValue, receiverObjectId, '1');
        }, 1000);
        return;
    }

    if (configKey == TASTER_CFG_HOLD_TIMEOUT) {
        holdTimeout = parseInt(newValue);
    } else if (configKey == TASTER_CFG_DOUBLE_CLICK_TIMEOUT) {
        doubleClickTimeout = parseInt(newValue);
    } else if (
        configKey == TASTER_CFG_EVENT_PRESSED_ENABLED ||
        ...
        if (newValue == true) {
            events = setBit(events, myConfigEventBits[configKey]);
        } else if (newValue == false) {
            events = clearBit(events, myConfigEventBits[configKey]);
        }
    } else if (configKey == TASTER_CFG_INVERTED) {
        if (newValue == true) {
            options = setBit(options, myConfigOptionsBits[configKey]);
        } else if (newValue == false) {
            options = clearBit(options, myConfigOptionsBits[configKey]);
        }
    }
...
function hwIrSensorReceivedEvents(sender, receiver, functionId, message, dataLength) {
    ...
    if (functionId == 202 || functionId == 203 || functionId == 204) {
        var address = bytesToWord(message, pos);
        pos += 2;
        var command = bytesToWord(message, pos);
        pos += 2;
...
function hwSchalterOff(offDelay, receiverObjectId) {
    ...
    var pos = 0;
    data[pos++] = 2; // Funktion ID
    wordToByteArray(offDelay, data, pos);
    pos += 2;
...
function hwSchalterOn(duration, onDelay, receiverObjectId) {
    ...
    var pos = 0;
    data[pos++] = 3; // Funktion ID
    wordToByteArray(duration, data, pos);
    pos += 2;
    wordToByteArray(onDelay, data, pos);
    pos += 2;
...
function hwSchalterToggle(offTime, onTime, quantity, receiverObjectId) {
    info(
        `relay toggle offTime = ${offTime}, onTime = ${onTime}, quantity = ${quantity} -> ${objectIdToString(
            receiverObjectId,
        )}`,
    );
...
function hwSchalterReceivedEvents(sender, receiver, functionId, message, dataLength) {
    ...
    if (functionId == 200) {
        ...
        event = 'evOn';
        var duration = bytesToWord(message, pos);
        pos += 2;
    } else if (functionId == 202) {
        event = 'evToggle';
    } else if (functionId == 203) {
        event = 'evCmdDelay';
        var cmdDelay = bytesToWord(message, pos);
        pos += 2;
...
function objectIdToString(objectId) {
    ...
    if (typeof moduleName != 'undefined') {
        ...
        moduleName = 'UNKNOWN_DEVICE';
    }

    return `${moduleName} (${deviceId}).${getInstanceName(deviceId, moduleType, classId, instanceId)}, moduleType=${
        moduleType
    }`;
}
...
function hwSchalterReceivedConfiguration(sender, receiver, message, dataLength) {
    ...
    var timeBase = bytesToWord(message, pos);
    pos += 2;
...
function hwRolloSetConfiguration(configKey, newValue, receiverObjectId, recovery = '0') {
    ...
    if (typeof configuration != 'undefined') {
        ...
        setTimeout(function () {
            hwRolloSetConfiguration(configKey, newValue, receiverObjectId, '1');
        }, 1000);
        return;
    }

    if (configKey == ROLLO_CFG_CLOSE_TIME) {
        closeTime = parseInt(newValue);
    } else if (configKey == ROLLO_CFG_OPEN_TIME) {
        openTime = parseInt(newValue);
...
function hwLogicalButtonBlink(brightness, offTime, onTime, quantity, receiverObjectId) {
    info(
        `backlight blink brightness = ${brightness}, offTime = ${offTime}, onTime = ${onTime}, quantity = ${
            quantity
        } -> ${objectIdToString(receiverObjectId)}`,
    );
...
function hwLogicalButtonSetMinBrightness(minBrightness, duration, receiverObjectId) {
    info(
        `backlight setMinBrightness minBrightness = ${minBrightness}, duration = ${duration} -> ${objectIdToString(
            receiverObjectId,
        )}`,
    );
...
function hwLedOn(brightness, duration, onDelay, receiverObjectId) {
    ...
    info(
        `output on brightness = ${brightness}, duration = ${duration}, onDelay = ${onDelay} -> ${objectIdToString(
            receiverObjectId,
        )}`,
    );
...
function hwLedBlink(brightness, offTime, onTime, quantity, receiverObjectId) {
    ...
    info(
        `output blink brightness = ${brightness}, offTime = ${offTime}, onTime = ${onTime}, quantity = ${
            quantity
        } -> ${objectIdToString(receiverObjectId)}`,
    );
...
function hwLedReceivedEvents(sender, receiver, functionId, message, dataLength) {
    ...
    if (event == 'evOff') {
        newState = false;
    } else if (event == 'evOn' || event == 'evBlink') {
        newState = true;
    }
...
function hwLedSetConfiguration(configKey, newValue, receiverObjectId, recovery = '0') {
    ...
    if (typeof configuration != 'undefined') {
        ...
        setTimeout(function () {
            hwLedSetConfiguration(configKey, newValue, receiverObjectId, '1');
        }, 1000);
        return;
    }
    ...
    } else if (configKey == LED_CFG_TIMEBASE) {
        timebase = newValue;
    }
    ...
    data[pos++] = dimmOffset;
    data[pos++] = minBrightness;
    wordToByteArray(timebase, data, pos);
    pos += 2;
    data[pos++] = options;
...
function hwDimmerReceivedEvents(sender, receiver, functionId, message, dataLength) {
    ...
    if (functionId == 200 || functionId == 201) // evOff u. evOn
    {
        ...
        if (functionId == 201) {
            ...
            info(`dimmer event evOff <- ${objectIdToString(sender)}`);
        }
...
function hwDimmerSetBrightness(brightness, duration, receiverObjectId) {
    ...
    info(
        `dimmer setBrightness brightness = ${brightness}, duration = ${duration} -> ${objectIdToString(
            receiverObjectId,
        )}`,
    );
...
function hwDimmerReceivedConfiguration(sender, receiver, message, dataLength) {
    ...
    var mode = '';
    if (modeByte == 0) {
        mode = DIMMER_MODE_TRAILING_EDGE;
    } else if (modeByte == 1) {
        mode = DIMMER_MODE_LEADING_EDGE;
    } else if (modeByte == 2) {
        mode = DIMMER_MODE_SWITCH_ONLY;
    }
    ...
    configurations[sender] = {
        ...
        dimmingTime: dimmingTime,
        dimmingRangeStart: dimmingRangeStart,
        dimmingRangeEnd: dimmingRangeEnd,
    };
...
function hwDimmerSetConfiguration(configKey, newValue, receiverObjectId, recovery = '0') {
    ...
    if (typeof configuration != 'undefined') {
        ...
        setTimeout(function () {
            hwDimmerSetConfiguration(configKey, newValue, receiverObjectId, '1');
        }, 1000);
        return;
    }

    if (configKey == DIMMER_CFG_MODE) {
        if (newValue == DIMMER_MODE_TRAILING_EDGE) {
            mode = 0;
        } else if (newValue == DIMMER_MODE_LEADING_EDGE) {
            mode = 1;
        } else if (newValue == DIMMER_MODE_SWITCH_ONLY) {
            mode = 2;
        }
    } else if (configKey == DIMMER_CFG_FADING_TIME) {
        fadingTime = parseInt(newValue);
    } else if (configKey == DIMMER_CFG_DIMMING_TIME) {
        dimmingTime = parseInt(newValue);
    } else if (configKey == DIMMER_CFG_DIMMING_RANGE_START) {
        dimmingRangeStart = parseInt(newValue);
    } else if (configKey == DIMMER_CFG_DIMMING_RANGE_END) {
        dimmingRangeEnd = parseInt(newValue);
    }
...
function hwRgbDimmerReceivedEvents(sender, receiver, functionId, message, dataLength) {
    ...
    if (functionId == 200 || functionId == 201) // evOff u. evOn
    {
        ...
        if (functionId == 201) {
            ...
            info(
                `rgb dimmer event evOn brightnessRed = ${brightnessRed}, brightnessGreen = ${
                    brightnessGreen
                }, brightnessBlue = ${brightnessBlue} <- ${objectIdToString(sender)}`,
            );
        } else {
            info(`rgb dimmer event evOff <- ${objectIdToString(sender)}`);
        }
...
function hwRgbDimmerSetColor(red, green, blue, duration, receiverObjectId) {
    ...
    info(
        `rgbDimmerSetColor red = ${red}, green = ${red}, green = ${red}, blue = ${blue}, duration = ${
            duration
        } -> ${objectIdToString(receiverObjectId)}`,
    );
...
function hwRgbDimmerSetConfiguration(configKey, newValue, receiverObjectId, recovery = '0') {
    ...
    if (typeof configuration != 'undefined') {
        ...
        setTimeout(function () {
            hwRgbDimmerSetConfiguration(configKey, newValue, receiverObjectId, '1');
        }, 1000);
        return;
    }

    if (configKey == RGB_DIMMER_CFG_FADING_TIME) {
        fadingTime = parseInt(newValue);
    }
...
function hwEthernetSetConfiguration(newValue, receiverObjectId, recovery = '0') {
    ...
    if (typeof configuration != 'undefined') {
        ...
        setTimeout(function () {
            hwEthernetSetConfiguration(configKey, newValue, receiverObjectId, '1');
        }, 1000);
        return;
    }
    ...
    info(
        `ethernet setConfiguration: ${dump(configurations[receiverObjectId])} -> ${objectIdToString(receiverObjectId)}`,
    );

    if (changes) {
        hwControllerReset(getObjectId(getDeviceId(receiverObjectId), CLASS_ID_CONTROLLER, 1));
    }
}
...
function hwEthernetReceivedConfiguration(sender, receiver, message, dataLength) {
    ...
    if (isBitSet(options, CONFIG_BITS[CLASS_ID_ETHERNET]['options'][ETHERNET_CFG_FIXED_IP_DHCP])) {
        ip = 'DHCP';
    }
    setStateIoBroker(myId, ip);
}
...
function hwControllerReceivedModuleId(sender, receiver, message, dataLength) {
    ...
    info(
        `controller moduleId: name = ${name}, size = ${size}, version =  ${version}, type = ${
            firmwareId
        } <- ${objectIdToString(sender)}`,
    );
...
function hwControllerReceivedRemoteObjects(sender, receiver, message, dataLength) {
    ...
    info(
        `functions for deviceId ${deviceId}: nrRelais = ${nrRelais}, nrTaster = ${nrTaster}, nrLeds = ${nrLeds}, nrDimmer = ${nrDimmer}, analogInput = ${nrAnalogInput}`,
    );

    var moduleType = '';

    if (typeof moduleTypes[deviceId] == 'undefined') {
        if (nrRelais == 17 && nrTaster == 16) {
            moduleType = MODUL_ID_32_IO;
        } else if (nrRelais == 17 && nrTaster == 8) {
            moduleType = MODUL_ID_16_RELAIS_V2;
        } else if (nrRelais == 17) {
            moduleType = MODUL_ID_16_RELAIS_V1;
        } else if (nrTaster == 12 && nrLeds == 12 && nrRelais == 1) {
            moduleType = MODUL_ID_24_UP_IO;
        } else if (nrTaster == 4 && nrLeds == 4 && nrRelais == 1) {
            moduleType = MODUL_ID_4_DIMMER;
        } else if ((nrTaster == 6 && nrLeds == 6) || (nrTaster == 12 && (nrLeds == 12 || nrLeds == 13))) {
            moduleType = MODUL_ID_6_TASTER;
        } else if ((nrTaster == 4 && nrLeds == 4) || (nrTaster == 10 && nrLeds == 10)) {
            moduleType = MODUL_ID_4_TASTER;
        } else if ((nrTaster == 2 && nrLeds == 2) || (nrTaster == 8 && nrLeds == 8)) {
            moduleType = MODUL_ID_2_TASTER;
        } else if ((nrTaster == 1 && nrLeds == 1) || (nrTaster == 7 && nrLeds == 7)) {
            moduleType = MODUL_ID_1_TASTER;
        } else if (nrRelais == 9) {
            moduleType = MODUL_ID_8_RELAIS;
        } else if (nrRelais == 12) {
            moduleType = MODUL_ID_12_RELAIS;
        } else if (nrRollos == 8) {
            moduleType = MODUL_ID_8_ROLLO;
        } else if (nrDimmer == 8 && nrRelais == 1) {
            moduleType = MODUL_ID_8_DIMMER;
            ...
            moduleType = MODUL_ID_2_ROLLOS;
        }

        if (moduleType == '' || typeof MODULES[moduleType] == 'undefined') {
            warn(
                `A unrecognized module type for deviceId ${deviceId} aborting: nrRelais = ${nrRelais}, nrTaster = ${
                    nrTaster
                }, nrLeds = ${nrLeds}, nrDimmer = ${nrDimmer}, analogInput = ${nrAnalogInput}`,
            );
            return;
        }

        warn(
            `B unrecognized module type for deviceId ${deviceId} using autodetected type: ${
        ...
        if (moduleType == MODUL_ID_16_RELAIS_V2 && nrRollos == 8) {
            moduleType = MODUL_ID_8_ROLLO;
        }
    }
    ...
    // ID
    adapter.setObjectNotExists(`${adapter.namespace}.${deviceId}`, {
        type: 'device',
        common: { name: moduleType },
        native: {},
    });
    ...
    parts.forEach(function (item) {
        ...
        if (classId == CLASS_ID_ETHERNET) {
            ...
            addStateIoBroker(
                ...
                false,
                true,
                hwEthernetGetCurrentIp(ethernetObjectId),
                CHANNEL_CONFIG,
            );
            addStateIoBroker(
                ETHERNET_CFG_FIXED_IP_DHCP,
                'string',
                'text',
                deviceId,
                CLASS_ID_CONTROLLER,
                ...
                true,
                true,
                hwEthernetGetConfiguration(ethernetObjectId),
                CHANNEL_CONFIG,
            );
        }
        ...
        // CLASS
        adapter.setObjectNotExists(`${adapter.namespace}.${deviceId}.${className}`, {
            type: 'channel',
            common: { name: className },
            native: {},
        });

        // Instance
        var instanceName = getInstanceName(deviceId, moduleType, classId, instanceId);
        adapter.setObjectNotExists(`${adapter.namespace}.${deviceId}.${className}.${instanceName}`, {
            type: 'channel',
            common: { name: instanceName },
            native: {},
        });
...
function hwControllerReceivedConfiguration(sender, receiver, message, dataLength) {
    ...
    if (firmwareType == FIRMWARE_ID_HBC) {
        if (fcke == 0) {
            ...
            moduleId = MODUL_ID_8_RELAIS;
        } else if (fcke == 0xc) {
            moduleId = MODUL_ID_16_RELAIS_V2;
        } else if (fcke == 0x10) {
            moduleId = MODUL_ID_24_UP_IO;
        } else if (fcke == 0x12) {
            moduleId = MODUL_ID_8_ROLLO;
        } else if (fcke == 0x18) {
            moduleId = MODUL_ID_6_TASTER;
        } else if (fcke == 0x19) {
            moduleId = MODUL_ID_4_TASTER;
        } else if (fcke == 0x1a) {
    ...
    debug(
        `controller configuration: ${moduleTypeName} (${moduleId}), startupDelay = ${
            startupDelay
        }, logicalButtonMask = ${logicalButtonMask}, deviceId = ${deviceId}, reportMemoryStatusTime = ${
            reportMemoryStatusTime
        }, slotTypeA = ${slotTypeA}, slotTypeB = ${slotTypeB}, slotTypeC = ${slotTypeC}, slotTypeD = ${
            slotTypeD
        }, slotTypeE = ${slotTypeE}, slotTypeF = ${slotTypeF}, slotTypeG = ${slotTypeG}, slotTypeH = ${
            slotTypeH
        }, timeCorrection = ${timeCorrection}, dataBlockSize = ${dataBlockSize}, fcke = ${fcke}`,
    );
...
function addStateIoBroker(
    ...
    if (unit != '') {
        adapter.setObjectNotExists(ioBrokerId, {
            type: 'state',
            common: { name: name, type: type, role: role, write: writeable, read: readable, unit: unit },
            native: {},
        });
    } else {
        adapter.setObjectNotExists(ioBrokerId, {
            type: 'state',
            common: { name: name, type: type, role: role, write: writeable, read: readable },
            native: {},
        });
    }
    if (typeof ioBrokerStates[ioBrokerId] == 'undefined') {
        ioBrokerStates[ioBrokerId] = '-';
    }

    if (defaultValue != null) {
        setStateIoBroker(ioBrokerId, defaultValue);
    }
}
...
function addIoBrokerStatesForInstance(deviceId, classId, instanceId) {
    if (classId == CLASS_ID_CONTROLLER) {
        ...
        addStateIoBroker(
            ...
            false,
            true,
            `${firmwareTypes[deviceId]} ${moduleVersions[deviceId]}`,
        );
        ...
        addStateIoBroker(
            SCHALTER_FKT_ON_DURATION_DELAY,
            'string',
            'switch',
            deviceId,
            classId,
        ...
        addStateIoBroker(
            TASTER_FKT_DISABLE_EVENTS_TIMEOUT,
            'number',
            'switch',
            deviceId,
            classId,
        ...
        addStateIoBroker(
            TASTER_FKT_ENABLE_DISABLE_EVENTS,
            'boolean',
            'switch',
            deviceId,
            classId,
        ...
        addStateIoBroker(
            TASTER_CFG_DOUBLE_CLICK_TIMEOUT,
            'number',
            'state ',
            deviceId,
            classId,
        ...
        addStateIoBroker(
            TASTER_CFG_EVENT_CLICKED_ENABLED,
            'boolean',
            'state ',
            deviceId,
            classId,
        ...
        addStateIoBroker(
            TASTER_CFG_EVENT_PRESSED_ENABLED,
            'boolean',
            'state ',
            deviceId,
            classId,
        ...
        addStateIoBroker(
            TASTER_CFG_EVENT_DOUBLE_CLICKED_ENABLED,
            'boolean',
            'state ',
            deviceId,
            classId,
        ...
        addStateIoBroker(
            TASTER_CFG_EVENT_HOLD_END_ENABLED,
            'boolean',
            'state ',
            deviceId,
            classId,
        ...
        addStateIoBroker(
            TASTER_CFG_EVENT_HOLD_START_ENABLED,
            'boolean',
            'state ',
            deviceId,
            classId,
        ...
        addStateIoBroker(
            TASTER_CFG_EVENT_RELEASED_ENABLED,
            'boolean',
            'state ',
            deviceId,
            classId,
        ...
        addStateIoBroker(
            DIMMER_FKT_BRIGHTNESS_DURATION,
            'string',
            'state',
            deviceId,
            classId,
        ...
        addStateIoBroker(
            DIMMER_CFG_DIMMING_RANGE_START,
            'number',
            'state',
            deviceId,
            classId,
        ...
        addStateIoBroker(
            LED_FKT_ON_BRIGHTNESS_DURATION_ONDELAY,
            'string',
            'switch',
            deviceId,
            classId,
        ...
        addStateIoBroker(
            ...
            true,
            false,
            'brightness_offtime_ontime_quantity',
        );
        addStateIoBroker(
            LED_FKT_MIN_BRIGHTNESS,
            'number',
            'state',
            deviceId,
            classId,
        ...
        addStateIoBroker(
            LOGICAL_BUTTON_ON_BRIGHTNESS_DURATION,
            'number',
            'switch',
            deviceId,
            classId,
        ...
        addStateIoBroker(
            LOGICAL_BUTTON_FKT_BLINK_BRIGHTNESS_OFFTIME_ONTIME_QUANTITY,
            'string',
            'switch',
            deviceId,
            classId,
        ...
        addStateIoBroker(
            LOGICAL_BUTTON_FKT_MIN_BRIGHTNESS,
            'number',
            'state',
            deviceId,
            classId,
...
function hwControllerReset(receiverObjectId) {
    ...
    setStateIoBroker(
        getIoBrokerId(getDeviceId(receiverObjectId), CLASS_ID_CONTROLLER, 1, CONTROLLER_FKT_STATE_ONLINE),
        false,
    );
}
...
function updateDownloadedFirmware(receiverObjectId, filename) {
    ...
    setTimeout(function () {
        pongCallback[deviceId] = function () {
            updateDownloadedFirmwareB(receiverObjectId, filename);
        };
        hwControllerPing(objectIdBooter);
    }, 1200);
}
        ...
        case 'object':
            ...
            if (v === null) {
                ...
                for (var i = 0; i < v.length; i++) {
                    out += `${repeatString('   ', recursionLevel)}   [${i}]:  ${dump(
                        v[i],
                        'none',
                        recursionLevel + 1,
                    )}\n`;
                ...
                for (var member in v) {
                    ...
                    sContents += `${repeatString('   ', recursionLevel)}   ${member}:  ${dump(
                        v[member],
                        'none',
                        recursionLevel + 1,
                    )}\n`;
...
function getIoBrokerId(deviceId, classId, instanceId, propertyName, subChannel = '') {
    ...
    return `${adapter.namespace}.${deviceId}.${CLASSES[classId].name}.${getInstanceName(
        deviceId,
        moduleType,
        classId,
        instanceId,
    )}.${subChannel}${propertyName}`;
}
